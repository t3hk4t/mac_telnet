...
int raw_term() {
	...
	memcpy(&new_t, &orig_term, sizeof(struct termios) );

	/* raw mode, from tcsetattr man page */
	new_t.c_iflag &= ~(IGNBRK|BRKINT|PARMRK|ISTRIP|INLCR|IGNCR|ICRNL|IXON);
	new_t.c_lflag &= ~(ECHO|ECHONL|ICANON|ISIG|IEXTEN);
	new_t.c_cflag &= ~(CSIZE|PARENB);
	new_t.c_cflag |= CS8;
